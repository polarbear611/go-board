...
- **dev 模式下 `/api` 不可用**：`vite.config.js` **没有配 proxy**，而 Vite 的 SPA fallback 会把 `/api/problems` 也回成 `index.html`（HTTP **200** + `text/html`）。所以 `api/problems.js` 里的 `res.ok` 判断过不了关，报错是 `Unexpected token '<', "<!doctype "... is not valid JSON` 而不是干净的连接失败。要在本地调题目保存/加载，得另起后端并给 vite 加 `server.proxy['/api'] → http://localhost:8000`。
