...
| 手筋专项训练 | `tesuji` | `手筋-` | **24**（265–288） | 目录已全本录入：5 单元 18 小节 **128 组 768 题**；题目在陆续上传 |
...
与 5 道自己起名的 `有眼杀无眼-248…252`——按手筋册目录它们其实落在第 1 单元某小节的最后一页
（187–252 那一段），哪天想归位，改名成 `手筋-248…252` 即可。

`手筋-259` 原先也在这一堆里（灌库时从旧 `259` 改名而来，就是后来定下 `prefix` 规则的那次撞车），
手筋册目录补全后它已归位到**第 46 页**那一组。顺带发现**它的棋形是错的**：
线上记录在 A4 多了一颗黑子（几何识别 + 放大原图两边都确认 A4 是空的），待修。
