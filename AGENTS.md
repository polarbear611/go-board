...
| 手筋专项训练 | `tesuji` | `手筋-` | **132**（157–288 连续） | 目录已全本录入：5 单元 18 小节 **128 组 768 题**；题目在陆续上传 |

手筋册的目录是从书前「目录」页推出来的——那页给的是**页码**，不是题号。推算模型：
每个单元开头 2 页是讲解、没有编号题，其余每页正好 6 道、全书连续编号，
共 128 个题页 768 题。在三处独立锚点上验证过：p29→157、p46→259、p49→265，
且页首标题自带的序号（`<小节名>（十八）` 之类）也都对得上。
`group.page` 存了每组在书上的页码，拍照时用来对页。

另有 7 道不在任何目录内，归「最近保存」：`167` / `170`（上上册的题，两册目录都不含），
以及 5 道自己起名的 `有眼杀无眼-248…252`。**后 5 道现在是重复记录**——同一批书页录入后，
它们已经以 `手筋-248…252` 的正式题号在目录里了，旧别名那 5 条可以删。

`手筋-259` 原先也在这一堆里（灌库时从旧 `259` 改名而来，就是后来定下 `prefix` 规则的那次撞车），
目录补全后已归位到第 46 页那一组。它的棋形原先是错的（线上记录在 A4 多一颗黑子、又漏了 C6），
2026-09-13 随这批一起改正。

两册都已人工逐题核对完成，基准分别在 `eval/ground-truth.shihuo.json`（582 道，2026-09-09）
与 `eval/ground-truth.tesuji.json`（132 道，2026-09-13），`confidence` 全是 `verified`。

手筋册 157–264 那 108 道是第一批**做过的书页**（红笔批改、页面拱起），成绩反而比死活册好：

| | 死活册 582 道 | 手筋册 108 道 |
|---|---|---|
| 逐子准确率 | 96.29% | **98.24%**（2279 子错 40） |
| 逐题正确率 | 76.8% | **87.0%**（108 道错 14） |

**14 处错里有 12 处是左侧 A–C 列漏掉的白子**，与 `read_tile.py` 已知的弱点完全吻合。
另有 1 道（`手筋-174`）是**整盘错位一行**——平移一行后 23 子中 22 子吻合。
它躲过了切图的四道修复、躲过了无气块检查、也躲过了「最低行 ≥3」的离群扫描
（错位后底行仍有子），**只有人工逐题看图才发现**。收紧底部留白也修不掉，成因在
read_tile 自己定网格那一步。这就是流程里那步人工复核不能省的直接证据。
...
python3 eval/inkgray.py <照片> <出图>                       # 看红笔擦得干不干净
```

#### 切图的四个坑（都会产出「看着正常但是错的」结果）

孩子做过的书页比新书难切得多。这四条都是实际踩出来的，改 `slice_page.py` 前先读：

1. **红笔跨图**——批改的红弧线常越过两个图之间的空白，把它们连成一个连通块，
   6 个图变 5 个，整页切不开。`inkgray.py` 按**饱和度**（R−max(G,B)）擦掉彩色笔迹；
   **不能靠亮度**：实测红笔 L≈89、纸面 211、黑印刷 48，红笔比黑印刷浅但远深于纸面，
   任何亮度阈值都分不开。铅笔是灰的，擦不掉，仍可能被读成子。
2. **连通块断裂**——书页拱起 + 阴影时细网格线过不了阈值，一个图的外接矩形短一大截
   （实测 p32 第 179 题只剩 739px，同页其余五块 1100 上下）。`GROWS` 逐档加大膨胀重试，
   再拿同行/同列健康的块把**明显残缺的那一块**量回来。
3. **修复别过头**——早先按行列取并集统一尺寸，把健康的块也撑大了，边上带进隔壁图的一角，
   read_tile 锁错网格整题读串（第 275 题）。所以只修不足 0.85 的，且一律夹回自己的行列格子。
4. **下方留白够到页脚**——底排没有下邻居，为带上题号留的空白会伸进「日期／得分／签名」
   那张表，它的横线被当成棋盘行，整题往上错 2–3 行（第 157/161/168 题）。
   按行间距的**中点**外推一条边界封住；代价是题号常被切掉，对照图标题里有。

这四条全是**静默失败**：切出来的图自成一体，read_tile 在上面能定出网格、给出一个自洽的
结果，不比对照图根本看不出来。所以对照图这一步不能省。
